      ...
      <template v-if="elem.type === 'role'">
        <li v-for="item in elem.data" :key="item.roleId" 
          ...
          <a :title="item.description" :class="{active: elem.isActiveItem && elem.isActiveItem(item)}">
            <img src="@/assets/workflow/icon_role.png">{{item.roleName}}
          </a>
        </li>
      </template>
      <template v-if="elem.type === 'department'">
        <li v-for="item in elem.data" :key="item.id" class="check_box" :class="{not: !elem.isDepartment}">
          <a v-if="elem.isDepartment" 
            ...
            @click="elem.change(item)">
              <img src="@/assets/workflow/icon_file.png">{{item.departmentName}}</a>
          <a v-else><img src="@/assets/workflow/icon_file.png">{{item.departmentName}}</a>
      ...
      <template v-if="elem.type === 'employee'">
        <li v-for="item in elem.data" :key="item.id" class="check_box">
            <a :class="elem.isActive(item) && 'active'" 
              ...
              <img src="@/assets/workflow/icon_people.png">{{item.employeeName}}
            </a>
        </li>
      </template>
    </template>
...
.select-box {
  ...
  li {
    ...
    i {
      ...
      font-size: 12px;
      cursor: pointer;
      background: url(~@/assets/workflow/next_level_active.png) no-repeat 10px center;
      border-left: 1px solid rgb(238, 238, 238);
    }

    a.active+i {
      color: rgb(197, 197, 197);
      background-image: url(~@/assets/workflow/next_level.png);
      pointer-events: none;
    }
